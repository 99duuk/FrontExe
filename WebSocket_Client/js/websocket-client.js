...
const server_address = `ws://${SERVER_IP}:${SERVER_PORT}`; // 서버 주소 설정 (WebSocket 프로토콜 사용)

const socket = new WebSocket(server_address); // WebSocket 객체 생성 및 서버에 연결
...
const displayPacketMessage = function ($parentSelector, message) {
    ...
    switch (msgObj.cmd) {
        ...
        case 'allchat':
            if ('result' in msgObj)
                msg = msgObj.result === 'ok' ? '채팅 전송 성공' : '채팅 전송 실패'; // 채팅 전송 결과 메시지
            else if ('id' in msgObj)
                msg = `${msgObj.id} => ${msgObj.msg}`; // 채팅 메시지 출력
            break;
    }
